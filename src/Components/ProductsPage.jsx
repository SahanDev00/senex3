...
const ProductsPage = () => {
  ...
  useEffect(() => {
    ...
    const url = `https://admin.senex.lk/API/Item?CategoryMainID=${categoryName}&CategorySubID=${
      subCategoryID || ""
    }&BrandID=`;
      ...
      .then((result) => {
        if (result.success && Array.isArray(result.data)) {
          ...
          if (result.data.length > 0) {
            setCategoryMainName(
              result.data[0].categoryMainName || "Default Category"
            );
          } else {
