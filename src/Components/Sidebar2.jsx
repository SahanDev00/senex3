...
const Sidebar2 = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://admin.senex.lk/API/CategoryMain", {
      method: "GET",
      headers: {
        APIKey: apiKey,
      },
  ...
  const toggleCollapse = (category) => {
    ...
    if (!subCategories[category]) {
      ...
      const apiKey = process.env.REACT_APP_API_KEY;
      const url = `https://admin.senex.lk/API/CategorySub?CategoryMainID=${
        categories.find((cat) => cat.categoryMainName === category)
          ?.categoryMainID
      }`;
  ...
  const handleSubCategoryClick = (subCategory) => {
    ...
    setLoading(true);
    const apiKey = process.env.REACT_APP_API_KEY;
    const url = `https://admin.senex.lk/Api/Item?CategorySubID=${subCategory.categorySubID}`;
    ...
    <div className="font-poppins w-full h-screen">
      <div className="w-full border-red-600 border xl:ml-36 h-screen overflow-y-auto bg-black text-white">
        <div className="w-full h-[50px] border bg-red-700 border-white">
          <h1 className="flex items-center justify-center text-white w-full h-full uppercase font-semibold text-lg">
            Categories
          </h1>
        </div>
            ...
            {categories.map((category, index) => (
              <li key={index}>
                <Link to={`/product/products/${category.categoryMainID}`}>
                  <h3
                    className="flex items-center cursor-pointer mt-1 mb-2 font-semibold text-sm uppercase"
                ...
                {collapsedSections[category.categoryMainName] && (
                  <ul className="">
                    {subCategories[category.categoryMainName]?.map(
                      (subCategory, subIndex) => (
                        ...
                        >
                          <li
                            className="mb-2 uppercase ml-9 text-sm cursor-pointer text-white"
                            key={`${index}-${subIndex}`}
                          >
