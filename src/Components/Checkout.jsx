...
const Checkout = () => {
  ...
  const calculateItemsAndPieces = () => {
    ...
    const pieces = cartItems.reduce(
      (total, item) => total + (parseInt(item.quantity, 10) || 1),
      0
    );
    return { items, pieces };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const orderResponse = await fetch("https://admin.senex.lk/Api/Order", {
        method: "POST",
        headers: {
      ...
      if (orderResponse.ok) {
        ...
        for (const item of cartItems) {
          ...
          const orderItemResponse = await fetch(
            "https://admin.senex.lk/Api/OrderItem",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
        ...
        const orderActionResponse = await fetch(
          "https://admin.senex.lk/Api/OrderAction",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (orderActionResponse.ok) {
          ...
          alert(
            `Error: ${
              orderActionResult.errorMessage || "Failed to log order action."
            }`
          );
        }
      } else {
      ...
      if (customerDetails) {
        ...
        try {
          ...
          const response = await fetch(
            `https://admin.senex.lk/Api/Customer/${parsedDetails.customerID}`,
            {
              headers: {
                "Content-Type": "application/json",
                APIKey: apiKey,
  ...
  return (
    <div className="w-full xl:min-h-screen pt-28 mx-auto">
      <Helmet>
        <title>SENEX | Checkout</title>
      </Helmet>
      <div className="w-full mx-auto relative mb-7 md:mb-20">
        <h1 className="mt-10 md:mt-20 md:mb-5 text-2xl md:text-4xl text-white font-bold text-center font-poppins">
          Place Order
        </h1>
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2 mx-auto w-[80%]">
          <div className="mx-auto w-full">
            <form
              className="w-full bg-black/50 mt-5 md:mt-10 border-white border-4 rounded-xl p-5"
              onSubmit={handleSubmit}
            >
              <h1 className="text-2xl md:text-3xl font-bold text-center my-2 text-white font-poppins">
                Billing Details
              </h1>
              ...
              <button
                type="submit"
                className="w-full bg-red-500 py-2 rounded-xl font-poppins mt-7 hover:bg-red-600 text-white font-semibold"
              >
                Place Order
              </button>
            </form>
          ...
          <div className="w-full mx-auto">
            <div className="w-full flex items-center h-full justify-center md:mt-0 mt-5">
              <div className="w-full md:w-[300px] xl:w-[400px] mx-auto bg-black/50 rounded-xl border-4 p-6 text-white">
                <h1 className="font-semibold text-2xl mb-1 font-poppins">
                  Your Order
                </h1>
