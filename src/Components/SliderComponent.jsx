...
const SliderComponent = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://senexadmin.worldpos.biz/Api/SlideBanner',{
      method: 'GET',
      headers: {
        'APIKey': apiKey,
      },
        ...
        {details.map((detail, index) => (
          <div key={detail.slideBannerID}>
            <div className='absolute z-10 mt-10 sm:mt-24 md:mt-16 lg:mt-10 xl:mt-14 2xl:mt-14 mxl:mt-8 xl:ml-8 sm:ml-14 w-[350px]'>
              ...
              <button className={`sm:px-8 px-8 py-2 rounded-xl mt-4 text-sm sm:text-lg  text-white bg-red-600 hover:scale-105 duration-300 active:scale-95 ${detail.buttonName ? '' : 'hidden'} ${currentSlide === index ? 'float-up' : ''}`}>
                {detail.buttonName}
              </button>
            </div>
            <div className='w-full h-[200px] bg-black/30 sm:h-[400px] md:h-[260px] relative xl:h-[240px] mxl:h-[250px] lg:h-[280px] rounded-lg 2xl:h-[400px]'>
              <img src={`https://senexadmin.worldpos.biz/Uploads/${detail.slideBannerID}.jpg`} alt="Banner" className='right-0 h-full w-full absolute object-cover rounded-lg' />
            </div>
          </div>
        ))}
      </Slider>
